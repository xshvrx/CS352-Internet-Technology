...
def send_reliable(cs, filedata, receiver_binding, win_size):
    ...
    while win_left_edge < INIT_SEQNO + content_len:
        ...
        while True:
            readable, writable, exceptional = select.select(inputs, [], [], RTO)
            if (readable):
                receiever_data, receiver_address = readable[0].recvfrom(100)
                ack_message = Msg.deserialize(receiever_data)
                print ("Received {}".format(str(ack_message)))
                if ack_message.ack > last_acked:
